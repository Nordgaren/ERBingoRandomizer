    ...
    public:
        ...
        public:
            MemoryScannerRegion(const MEMORY_BASIC_INFORMATION& info)
                ...
                if (!VirtualProtect(m_info.BaseAddress, m_info.RegionSize, PAGE_EXECUTE_READWRITE,
                                    &m_original_protection)) {
        ...
        MemoryScanner(HMODULE base) {
            ...
            for (uintptr_t next = 0; VirtualQueryEx(process, static_cast<void*>(next), &info, sizeof(info)) == sizeof(
                     info); next += info.RegionSize) {
                if (info.State == MEM_COMMIT) {
                    info.BaseAddress = static_cast<void*>(next);
        ...
        MemoryScanner()
            : MemoryScanner(static_cast<HMODULE>(NtCurrentPeb()->ImageBaseAddress)) {
        }
        ...
        std::optional<uintptr_t> find(const std::string& pattern) {
            for (const auto region_info : m_memory_regions) {
                ...
                const auto iter = std::search(std::execution::par, region_view.begin(), region_view.end(),
        ...
        bool replace_at(uintptr_t location, std::function<void(uintptr_t)> replace_callback) {
            ...
            if (!VirtualProtect(static_cast<void*>(location), 0x1000 /* PAGE_SIZE */, PAGE_EXECUTE_READWRITE,
                                &original_protection)) {
                throw std::runtime_error("Unable to change process memory protection flags");
            }

            replace_callback(location);
            VirtualProtect(static_cast<void*>(location), 0x1000, original_protection, &original_protection);
