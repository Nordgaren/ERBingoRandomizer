...
namespace modengine {
    ...
    public:
        ...
        virtual void register_patch(GameType type, const std::string_view& signature,
    ...
    protected:
        ...
        template <typename T>
        void register_hook(GameType type, Hook<T>* hook, const std::string& module, const std::string& function,
                           T detour) {
            ...
            auto addr = static_cast<uintptr_t>(GetProcAddress(mod, function.c_str()));
            if (addr == 0) {
                return;
            }

            hook->original = static_cast<T>(addr);
            hook->replacement = detour;

            m_ext_connector->register_hook(type, static_cast<Hook<GenericFunctionPointer>*>(hook));
        }
        ...
        template <typename T>
        void register_hook(GameType type, ScannedHook<T>* hook, const ScanPattern& signature, T detour,
                           HookScanMode mode) {
            hook->mode = mode;
            hook->pattern = signature;
            hook->replacement = detour;

            m_ext_connector->register_hook(type, static_cast<ScannedHook<GenericFunctionPointer>*>(hook));
        }
        ...
        void register_patch(GameType type, const std::string_view& signature,
                            std::function<void(uintptr_t)> replace_callback) {
